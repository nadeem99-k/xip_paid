...
export async function generateImage(prompt, initImgBuffer, mode) {
    ...
    const negative_base = "(extra limbs, extra legs, extra arms, three legs, fused bodies, connected persons, mutated anatomy, deformed body:1.5), (multiple people, group of people, twins, clones:1.6), (extra fingers:1.4), blurry, low quality, deformed, disfigured, ugly, bad anatomy, (plastic skin:2.5), (doll anatomy:2.5), (censored:2.5), (smoothed crotch:2.5), (airbrushed:1.5), fake body, 3d render, cgi, cartoon, anime, (changed face:1.5), (changed background:1.5), (extra heads:1.5).";
    ...
    if (mode === 'bikini') {
        ...
        finalPrompt += `${zero_touch_preservation} ${identity_preservation} ${background_preservation} ${pose_preservation} ${masterpiece_enhancer} (wearing a bikini:1.6), (FORCED BIKINI TRANSFORMATION:1.5). (STRICT COMPLIANCE: KEEP ORIGINAL FACE, POSE, STYLE, AND BACKGROUND 100% UNTOUCHED:1.8). (Ultra realistic natural human body:1.4). (Anatomic correctness:1.5). Real photography.`;
        negativePrompt += " artificial, 3d render, cartoon, digital art, airbrushed skin, plastic texture, smoothed face, fake lighting, changed background, modified face, (extra limbs:1.5), changed pose, modified pose, swapped pose, (unremoved clothes:1.6), (visible original clothes:1.6), changed style, illustration, painting.";
    } else if (mode === 'nude') {
